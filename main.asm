...
; Determina si el n�mero ingresado cuenta con parte entera. 
VerificarEnteros:
    CMP CX, 0
    JE  ErrorFormato
...
leer_entero:
    ...
    sub al, '0'              ; convierte un d�gito almacenado como car�cter ASCII en su equivalente num�rico.
    mov ah, 0
...
leer_decimal:
    ...
    ; Convertir ASCII a nï¿œmero
    sub al, '0'
    mov ah, 0
    push ax                  ; Guardar nuevo dï¿œgito
    ...
    ; temp_decimal = temp_decimal + nuevo_dï¿œgito
    pop ax
...
fin_numero:
    ...
    jmp procesar_numero      ; Procesar el siguiente nï¿œmero
...
SEGUIR_BS:    
    ...
    ; Al final array_BS queda con los �ndices 0, 1, 2, 3, 4, .... , contador - 1 
...
; Compara decimales                                          
;----------------------------------------  
; AX debe contener �ndice del elemento tal que el siguiente es con quien se debe compara
...
calcular_ponderado PROC
    ...
    XOR SI, SI              ; Reiniciar �ndice
    XOR AX, AX              ; Reiniciar acumulador
    MOV suma_total, 0
    
ciclo_suma:
    MOV BX, array_enteros[SI] ; Obtener calificaci�n entera
...
SUM_LOOP:
    ...
    add si, 4              ; siguiente n�mero (4 bytes)
    loop SUM_LOOP

    ; Ahora en DX:AX est� la suma de 32 bits
    ...
    ; DX ya contiene el resto de la divisi�n anterior
    div cx          ; AX = DX:AX / CX, DX = resto
...
CALCULO_LOOP:
    ...
    ; Actualizar residuo para siguiente iteraci�n
    mov si, dx
...
suma_de_decimales PROC
    ...
    ; Sumar la parte alta (16 bits m�s significativos) con acarreo
...
imprimir_promedio PROC
    ...
    ; Imprimir parte decimal (5 d�gitos fijos)
...
imprimir_numero PROC
    ...
    ; Si el n�mero es 0, imprimir '0' directamente
    CMP AX, 0
...
;-----------------------------------
;-----------------------------------
; Imprimir parte decimal (5 d�gitos)
;-----------------------------------
imprimir_decimales PROC
    ...
    MOV CX, 5                   ; siempre 5 d�gitos
siguiente_digito:
    ; cargar n�mero de 32 bits en DX:AX
...
MostrarListaMejores PROC
    ...
    mov cl, contador      ; n�mero de elementos (8 bits)
    cmp cl, 0
    je FIN_MLM            ; si no hay datos, salir

    xor si, si            ; �ndice en array_BS (en bytes)
BUCLE_MLM:
    mov ax, array_BS[si]  ; obtener valor (16 bits)
    mov al, al            ; si solo quieres la parte baja
                          ; (de lo contrario, p�salo en AX)
    call MostarPorID     ; llamar a funci�n con AL
