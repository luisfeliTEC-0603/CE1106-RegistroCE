...
INTERNO_LOOP:
    ...
    ; ComparaciÛn basada en el orden
    cmp orden, 1
    je ORDEN_ASC
    
    ; ORDEN DESCENDENTE (DSC = 0)
    cmp dx, bp
    jg NO_SWAP                ; Si i > j, est· bien (mayor a menor)
...
ORDEN_ASC:
    ...
    cmp dx, bp
    jl NO_SWAP                ; Si i < j, est· bien (menor a mayor)
...
COMPARE_DECIMALES_DSC:
    ...
    cmp dx, bp
    jg NO_SWAP                ; decimal[i] > decimal[j], est· bien
...
COMPARE_DECIMALES_ASC:
    ...
    cmp dx, bp
    jl NO_SWAP                ; decimal[i] < decimal[j], est· bien
